...
from transformers.models.pegasus.modeling_pegasus import (
    PegasusModel, shift_tokens_right, Seq2SeqModelOutput, PegasusConfig, PegasusPreTrainedModel, PegasusDecoder,
    CrossEntropyLoss, Seq2SeqLMOutput, PegasusAttention, PegasusEncoder, PegasusDecoderLayer, _expand_mask, BaseModelOutput,
    PegasusForConditionalGeneration, Optional, random
)

logger = logging.getLogger(__name__)


class RefSumPegasusEncoder(PegasusEncoder):
    def __init__(self, config: PegasusConfig, embed_tokens: Optional[nn.Embedding] = None):
        super(RefSumPegasusEncoder, self).__init__(config, embed_tokens)
...
# replace PegasusModel
class RefSumPegasusModel(PegasusModel):
    def __init__(self, config: PegasusConfig):
        super().__init__(config)
        self.encoder = RefSumPegasusEncoder(config, self.shared)
...
class PegasusForRefSumCG(PegasusForConditionalGeneration):
    def __init__(self, config: PegasusConfig):
        super().__init__(config)
        self.model = RefSumPegasusModel(config)
