...
from src.modules.summarisation.datasets import (
    BartDataset,
)
...
class SumBart(BaseTransformer):
    ...
    def _step(self, batch: dict):
        src_ids, src_mask = batch["doc"]['input_ids'], batch["doc"]["attention_mask"]
        tgt_ids = batch["target"]["input_ids"]

        outputs = self(input_ids=src_ids, attention_mask=src_mask, labels=tgt_ids, use_cache=False)
    ...
    def training_step(self, batch, batch_idx) -> Dict:
        ...
        logs["batch_size"] = batch["doc"]['input_ids'].shape[0]
    ...
    def get_dataset(self, data_name: str = "train") -> BartDataset:
    ...
    def get_dataloader(self, src_file_prefix: str, tgt_file_prefix: str, batch_size: int,
                       shuffle: bool = False) -> DataLoader:
        pass

    def get_ssn_dataloader(self, data_key: str, batch_size: int, shuffle: bool = False) -> DataLoader:
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        return self.get_ssn_dataloader("train", batch_size=self.hparams.train_batch_size,
                                   shuffle=train_shuffle)

    def val_dataloader(self) -> DataLoader:
        return self.get_ssn_dataloader("val", batch_size=self.hparams.eval_batch_size,
                                           shuffle=False)

    def test_dataloader(self) -> DataLoader:
        return self.get_ssn_dataloader("test", batch_size=self.hparams.eval_batch_size,
                                   shuffle=False)