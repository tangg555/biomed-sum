...
def mannwhitneyu_test(input_one:list, input_two:list, alternative='two-sided', output_p=True):
    from scipy.stats import mannwhitneyu
    results = mannwhitneyu(input_one, input_two, alternative=alternative)
    if output_p:
        return results.pvalue
    else:
        return results

def compute_metrics_for_summary(pred_lines, tgt_lines):
    decoded_preds = ["\n".join(nltk.sent_tokenize(pred.strip())) for pred in pred_lines]
    decoded_labels = ["\n".join(nltk.sent_tokenize(label.strip())) for label in tgt_lines]

    rouge_dict_result = calculate_rouge(pred_lines=decoded_preds,
                             tgt_lines=decoded_labels,
    ...
    # flatten
    result = {}
    for key, metric_dict in rouge_dict_result.items():
        for metric_type, val in metric_dict.items():
            result[f"{key}_{metric_type}"] = val
    result["avg_gen_len"] = np.mean(prediction_lens)
    return result
...
if __name__ == '__main__':
    result = compute_metrics_for_summary(pred_lines=["decoded_preds"],
                             tgt_lines=["decoded_labels"])
    print(result)