...
def save_lines(lines: list, path):
    with open(path, "w", encoding="utf-8") as fw:
        fw.writelines([f"{one}\n" for one in lines])

def load_jsonl(path) -> list:
    return list(jsonlines.open(Path(path), mode="r"))

def load_list(path) -> list:
    with open(path, "r", encoding="utf-8") as fr:
        return [one.strip() for one in fr.readlines()]


def save_jsonl(content: list, path):
    with open(path, "w", encoding="utf-8") as fw:
        for one in content:
            json.dump(one, fw)
            fw.write("\n")
...
def lines_to_file(lines: list, data_file):
